...
const PLUGIN_ID = 'dsh-concurrency-meter'
...
function createCssPlugin(pluginId: string) {
  ...
  return {
    ...
    resolveId(source: string, importer?: string) {
      ...
      const id = `${CSS_VIRTUAL_PREFIX}${pluginId}/${basename(absolute)}${CSS_VIRTUAL_SUFFIX}`
      const previous = files.get(id)
      if (previous !== undefined && previous !== absolute) {
        throw new Error(`CSS virtual module collision for ${basename(absolute)}`)
      }
    ...
    async load(id: string) {
      ...
      if (file === undefined) throw new Error(`Unknown CSS virtual module ${id}`)
